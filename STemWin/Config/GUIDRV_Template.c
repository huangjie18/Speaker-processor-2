...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1)
{
    ...
    if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR)
    {
        for (; y0 <= y1; y0++)
        {
            for (x = x0; x <= x1; x++)
            {
                _XorPixel(pDevice, x, y0); //当绘制模式为GUI_DM_XOR有效
            }
        }
    }
    else  //使用GUI_DM_NORMAL普通模式
    {
//    for (; y0 <= y1; y0++) {
//      for (x = x0; x <= x1; x++) {
...
//    }
		...
		Color = LCD_Color_trans(LCD_COLORINDEX);
...
*       Draw Bitmap 1 BPP
*/
static void _DrawBitLine1BPP(GUI_DEVICE * pDevice, int x, int y,
                             U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans)
{
...
*       Draw Bitmap 2 BPP
*/
static void  _DrawBitLine2BPP(GUI_DEVICE * pDevice, int x, int y,
                              U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans)
{
...
*       Draw Bitmap 4 BPP
*/
static void  _DrawBitLine4BPP(GUI_DEVICE * pDevice, int x, int y,
                              U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans)
{
    LCD_PIXELINDEX Pixels, PixelIndex;
    int CurrentPixel, Shift, Index;
...
*       Draw Bitmap 8 BPP
*/
static void  _DrawBitLine8BPP(GUI_DEVICE * pDevice, int x, int y,
                              U8 const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans)
{
...
*/
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y,
                              U16 const GUI_UNI_PTR * p, int xsize)
{
//  for (;xsize > 0; xsize--, x++, p++) {
//    _SetPixelIndex(pDevice, x, y, *p);
//  }
...
//    }
    ...
    for (; xsize > 0; xsize--, x++, p++)
    {
        ...
        *(__IO u16 *) (Bank1_LCD_D) = Color;  //加快速度
#else
        WriteData(Index);
#endif
    }
...
*/
static void _DrawBitLine32BPP(GUI_DEVICE * pDevice, int x, int y,
                              U32 const GUI_UNI_PTR * p, int xsize)
{
    for (; xsize > 0; xsize--, x++, p++)
    {
        _SetPixelIndex(pDevice, x, y, *p);
...
static void * _GetDevData(GUI_DEVICE * pDevice, int Index)
{
    ...
    switch (Index)
    {
        case LCD_DEVDATA_MEMDEV:
            return (void *)
                   &GUI_MEMDEV_DEVICE_16; // TBD: Has to be adapted to the right memory device depending on the used color depth!
    }
#else
    GUI_USE_PARA(Index);
#endif
...
static void _SetSize(GUI_DEVICE * pDevice, int xSize, int ySize)
{
    ...
    {
        pContext = (DRIVER_CONTEXT_TEMPLATE *)pDevice->u.pContext;
        pContext->vxSizePhys = (pContext->vxSizePhys == 0) ? xSize :
                               pContext->vxSizePhys;
